# afk enforcement guardrails

## Problem Statement

`afk` now has budget caps (hours, dollars, iterations) — but it has zero technical safeguards against the most common foot-guns of unattended runs:
...
Headline guarantee after this lands: *"afk refuses to start unless you have a PRD, your tree is clean, you're not in a known-dangerous directory, and you've eyeballed the issue queue."* Each "unless" can be waived explicitly.
...
5. As an engineer who legitimately needs to run `afk` in a sensitive directory, I want a `--i-know-this-is-dangerous` flag so I have to *name* what I'm waiving.
...
The work is one bash file edit (`scripts/afk`) — no new files. Conceptually it splits into:
...
False positives (e.g. `author-tools` matching `auth`) are accepted; the override is one keystroke away. Tuning the list later requires editing `afk` and re-installing — that's the point (auditable).
...
```
afk: warning — git: not a repo, dirty-tree check skipped
```
