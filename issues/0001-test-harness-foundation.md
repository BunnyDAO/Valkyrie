---
id: 0001
title: Test harness foundation for afk
type: AFK
status: done
blocked_by: []
...
A self-contained test harness that lets later slices verify `afk` end-to-end without making real LLM calls. The harness lives under a new `test/` directory in the repo root and provides:
...
- A runner script that puts the stubs first on PATH, sets up a clean temp working directory containing fixture `issues/` and `docs/prd/`, runs `afk` against them, and asserts on exit code, stdout, and the stage marker.
- A single smoke test: run today's `afk 2` against a stubbed `claude` that just emits one canned `usage` event per call, confirm the loop completes 2 iterations and clears the stage on exit. This proves the harness wires up correctly before any new feature builds on it.
...
- [x] Smoke test exercises real `afk` (no mocks of the script under test) against a stubbed CLI and asserts iteration count, exit code, and stage marker.
