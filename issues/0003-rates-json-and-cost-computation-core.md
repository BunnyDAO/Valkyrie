...
- A `load_rates` function in `ralph-afk` that reads `~/.claude/valkyrie/rates.json` once at startup, validates the shape (provider keys present, each model has the five required fields), and stores the parsed data in memory.
...
- [x] `ralph-afk` exits with a clear error if `~/.claude/valkyrie/rates.json` does not exist.
- [x] `ralph-afk` exits with a clear error if the file is malformed JSON.
- [x] `ralph-afk --debug-cost <fixture-log>` prints the parsed `(model, input, output, cw5m, cw1h, cread)` values and the computed USD cost for the given log file.
