...
- A `preflight()` function in `ralph-afk` that runs after env setup but before the startup banner. It calls each registered gate function in turn, collects failure messages into a `GATE_FAILURES` accumulator, and if any failed prints the punch list to stderr and exits non-zero.
...
- The first gate: `gate_prd_exists`. Returns 0 if `docs/prd/` exists AND contains at least one non-empty `.md` file. Otherwise returns 1 and appends to `GATE_FAILURES`:
  ...
  ```
- New flag: `--allow-no-prd` (no value). When passed, `gate_prd_exists` skips the check and prints a one-line warning before the startup banner: `ralph-afk: warning — PRD check skipped (--allow-no-prd)`.
...
- [x] `ralph-afk 2` in a workdir with no `docs/prd/` directory exits non-zero with the punch-list error and "no PRD found in docs/prd/" message.
- [x] `ralph-afk 2` in a workdir with `docs/prd/` but no files exits non-zero with the same message.
- [x] `ralph-afk 2` in a workdir with `docs/prd/x.md` (zero bytes) exits non-zero with the same message.
- [x] `ralph-afk 2` in a workdir with `docs/prd/x.md` containing text passes the gate and the loop runs.
- [x] `ralph-afk 2 --allow-no-prd` in a workdir with no PRD prints the override warning, passes the gate, and the loop runs.
