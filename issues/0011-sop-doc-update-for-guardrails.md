...
- **Add a new subsection in §7 titled "Pre-flight guardrails"** placed AFTER "Budget caps — what stops the loop" and BEFORE "Per-iteration progress and final summary". The subsection:
  - States the headline guarantee: *"afk refuses to start unless you have a PRD, your tree is clean, you're not in a known-dangerous directory, and you've eyeballed the issue queue."*
...
- **Add new rows to the failure-modes table in §8**:
  - `afk: cannot start — fix the following:` → "Pre-flight gate(s) failed. Read the punch list, fix each item, or pass the matching override flag."
...
- [x] The dangerous-path keyword list is reproduced verbatim from `scripts/afk` (no drift).
